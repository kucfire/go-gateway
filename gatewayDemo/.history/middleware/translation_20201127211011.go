...
func TranslationMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		default:
			...
			// 验证方法：dto.AppInfo.WhiteIPS
			val.RegisterValidation("vaild_white_ips", func(fl validator.FieldLevel) bool {
				// return fl.Field().String() == ""
				for _, ms := range strings.Split(fl.Field().String(), "\n") {
					if matched, _ := regexp.Match(`^\S+\:\d+$`, []byte(ms)); !matched {
						return false
			...
			val.RegisterTranslation("valid_secret", trans, func(ut ut.Translator) error {
				return ut.Add("valid_secret", "{0} 必须为非空字符", true)
			}, func(ut ut.Translator, fe validator.FieldError) string {
				t, _ := ut.T("valid_secret", fe.Field())
				return t
			})
			// 验证器：dto.AppInfo.WhiteIPS
			val.RegisterTranslation("vaild_white_ips", trans, func(ut ut.Translator) error {
