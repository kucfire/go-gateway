...
func TranslationMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		default:
			...
			// dto.AdminLoginInput.UserName
			val.RegisterValidation("vail_username", func(fl validator.FieldLevel) bool {
				return fl.Field().String() == "admin"
			})

			//自定义验证器
			//https://github.com/go-playground/validator/blob/v9/_examples/translations/main.go
			val.RegisterTranslation("vail_username", trans, func(ut ut.Translator) error {
				return ut.Add("vail_username", "{0} 填写不正确哦", true)
			}, func(ut ut.Translator, fe validator.FieldError) string {
				t, _ := ut.T("vail_username", fe.Field())
				return t
			})
			break
		}
