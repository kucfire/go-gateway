...
func (adminligin *ServiceController) ServiceAddHTTP(c *gin.Context) {
	...
	if err = serviceGRPCRule.Save(c, tx); err != nil {
		tx.Rollback()
	...
	// 存储TCPRule
	serviceTCPRule := &dao.ServiceTCPRule{
		ID: serviceModel.ID,
	}
	if err = serviceTCPRule.Save(c, tx); err != nil {
		tx.Rollback()
...
func (adminligin *ServiceController) ServiceAddGRPC(c *gin.Context) {
	...
	if _, err := tcpRule.Find(c, lib.GORMDefaultPool, tcpRule); err == nil {
		middleware.ResponseError(c, 2001, errors.New("服务端口被占用"))
		return
	}
	// 校验grpc的port
	...
	if _, err := grpcRule.Find(c, lib.GORMDefaultPool, grpcRule); err == nil {
