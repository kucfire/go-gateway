...
func (adminligin *ServiceController) ServiceUpdateGRPC(c *gin.Context) {
	...
	// 校验服务信息
	serviceInfoDetail = &dao.ServiceInfo{ID: params.ID}
	serviceDetail, err := serviceInfoDetail.ServiceDetail(c, tx, serviceInfoDetail)
	if err != nil {
		tx.Rollback()
		middleware.ResponseError(c, 2004, err)
		return
	...
	serviceInfo = &dao.ServiceInfo{
		...
		ServiceName: params.ServiceName,
		ServiceDesc: params.ServiceDesc,
		CreatedAt:   serviceDetail.Info.CreatedAt,
		IsDelete:    serviceDetail.Info.IsDelete,
	}
	if err := serviceInfo.Save(c, tx); err != nil {
		tx.Rollback()
		middleware.ResponseError(c, 2005, err)
...
// @Success 200 {object} middleware.Response{data=dto.ServiceAddTCPInput} "success"
// @Router /service/service_add_tcp [post]
func (adminligin *ServiceController) ServiceAddTCP(c *gin.Context) {
