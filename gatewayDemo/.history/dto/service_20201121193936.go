...
type ServiceAddHTTPInput struct {
	// 服务名称
	ServiceName string `json:"service_name" form:"service_name" comment:"服务名称" example:"" validate:"required, balid_service_name"`
	// 服务描述
	ServiceDesc string `json:"serbice_desc" form:"serbice_desc" comment:"服务描述" example:"" validate:"required"`
	// 接入类型
	RuleType int `json:"rule_type" form:"rule_type" comment:"接入类型" example:"" validate:""`
	// 接入路径
	Rule string `json:"rule" form:"rule" comment:"接入路径：域名或者前缀" example:"" validate:"required"`
	// 是否支持HTTPS, 1=支持
	NeedHTTPS int `json:"need_https" form:"need_https" comment:"是否支持HTTPS:1=支持" example:"" validate:""`
	// 是否启用strip_uri, 1=启用
	NeedStripURI int `json:"need_strip_uri" form:"need_strip_uri" comment:"启用strip_uri 1=启用" example:"" validate:""`
	// 是否支持websocket, 1=支持
	NeedWEBSocket int `json:"need_websocket" form:"need_websocket" comment:"是否支持websocket 1=支持" example:"" validate:""`
	// url重写功能 格式：^/gatekeeper/test_service(.*) $1 多个逗号间隔
	URLRewrite string `json:"url_rewrite" form:"url_rewrite" comment:"url重写功能 格式：^/gatekeeper/test_service(.*) $1 多个逗号间隔" example:"" validate:""`
	// header转换支持增加(add)、删除(del)、修改(edit) 格式：add headname headvalue 多个逗号间隔
	HeaderTransfor string `json:"header_transfor" form:"header_transfor" comment:"header转换支持增加(add)、删除(del)、修改(edit) 格式：add headname headvalue 多个逗号间隔" example:"" validate:""`
	// 是否开启权限 1=开启
	OpenAuth int `json:"open_auth" form:"open_auth" comment:"是否开启权限 1=开启" example:"" validate:""`
	// 黑名单ip
	...
	// 服务器限流
	ServerFlowLimit int `json:"service_flow_limit" form:"service_flow_limit" comment:"服务器限流" example:"" validate:""`
	// 轮询方式 0=random 1=round_robin 2=wieght_round_robin 3=ip_hash
	RoundType int `json:"round_type" form:"round_type" comment:"轮询方式 0=random 1=round_robin 2=wieght_round_robin 3=ip_hash" example:"" validate:""`
	// ip列表
	...
	// 建立连接超时，单位s
	UpstreamConnectTimeout int `json:"upstream_connect_timeout" form:"upstream_connect_timeout" comment:"建立连接超时，单位s" example:"" validate:""`
	// 获取header超时，单位s
	UpstreamHeaderTimeout int `json:"upstream_header_timeout" form:"upstream_header_timeout" comment:"获取header超时，单位s" example:"" validate:""`
	// 链接最大空闲时间，单位s
	UpstreamIdleTimeout int `json:"upstream_idle_timeout" form:"upstream_idle_timeout" comment:"链接最大空闲时间，单位s" example:"" validate:""`
	// 最大空闲链接数
	UpstreamMaxIdle int `json:"upstream_max_idle" form:"upstream_max_idle" comment:"最大空闲链接数" example:"" validate:""`
}
