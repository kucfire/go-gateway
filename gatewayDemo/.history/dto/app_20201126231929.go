...
type AppAddInput struct {
	// 服务名称
	AppID string `json:"app_id" form:"app_id" comment:"租户id" example:"" validate:"required,valid_service_name"`
	// 服务名称
	Name string `json:"name" form:"name" comment:"租户名称" example:"" validate:"required,valid_service_name"`
	// 服务名称
	Secret string `json:"secret" form:"secret" comment:"密钥" example:"" validate:"required,valid_service_name"`
	// 服务名称
	WhiteIPS string `json:"white_ips" form:"white_ips" comment:"ip白名单，支持前缀匹配" example:"" validate:"required,valid_service_name"`
}
