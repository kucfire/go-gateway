...
func InitRouter(middlewares ...gin.HandlerFunc) *gin.Engine {
	...
	// store, err := sessions.NewRedisStore(10, "tcp", "localhost:6379", "Udi", []byte("secret"))
	...
	adminInfoRouter := router.Group("/admin_info")
	adminInfoRouter.Use(
		sessions.Sessions("mysession", store),
		middleware.RecoveryMiddleware(),
		middleware.RequestLog(),
	...
	serviceListRouter := router.Group("/service")
	serviceListRouter.Use(
		sessions.Sessions("mysession", store),
		middleware.RecoveryMiddleware(),
		middleware.RequestLog(),
		middleware.SessionAuthMiddleware(),
	...
	{
		controller.ServiceRegister(serviceListRouter)
	}

	appListRouter := router.Group("/app")
	appListRouter.Use(
		sessions.Sessions("mysession", store),
		middleware.RecoveryMiddleware(),
		middleware.RequestLog(),
